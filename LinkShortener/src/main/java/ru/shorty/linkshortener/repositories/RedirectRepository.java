...
public interface RedirectRepository extends JpaRepository<LinkRedirectModel, UUID> {
    ...
    @Query("SELECT lr.deviceType AS name, COUNT(lr) AS value FROM LinkRedirectModel lr WHERE lr.link.uid = :linkUid GROUP BY lr.deviceType")
    List<Map<String, Object>> getGroupByDeviceType(@Param("linkUid") UUID linkUid);

    @Query("SELECT lr.osType AS name, COUNT(lr) AS value FROM LinkRedirectModel lr WHERE lr.link.uid = :linkUid GROUP BY lr.osType")
    List<Map<String, Object>> getGroupByOsType(@Param("linkUid") UUID linkUid);

    @Query("SELECT lr.browserType AS name, COUNT(lr) AS value FROM LinkRedirectModel lr WHERE lr.link.uid = :linkUid GROUP BY lr.browserType")
